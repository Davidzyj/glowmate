...
Use the camera preview to estimate brightness on your device, then apply a practical screen fill-light setup. Adjust brightness and tone, open a full-screen soft light, use scene presets, and restore recent local records.
...
App 会使用相机预览在设备本地估算亮度，并推荐实用的屏幕补光参数。你可以调节亮度和色调，开启全屏柔光，选择场景预设，并恢复最近使用过的本地记录。
...
カメラプレビューを使って端末内で明るさを推定し、実用的な画面補助光の設定を提案します。明るさと色を調整し、全画面ソフトライトを開き、シーンプリセットや最近のローカル履歴を利用できます。
...
- Notes:
GlowMate is a local-only iPhone app. It uses camera preview frames only on device to estimate lighting and recommend screen fill-light settings. It does not create accounts, contact a backend, include analytics/ads/tracking SDKs, or upload photos/video. The Privacy Policy and Support links are opened only after the user taps them.
